...
/** ZebraRfidLib */
public class ZebraRfidLib implements FlutterPlugin, MethodCallHandler, StreamHandler {
  ...
  private final String METHODCHANNEL = "support.lib.rfid/method";
  private final String EVENTCHANNEL = "support.lib.rfid/event";
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    Log.d("RFIDPlugin", "Received method------------------"+call.method);
    // get method
    ...
    switch (method) {
      ...
      case level:
        if (device != null) {
          int level = 0;
          try {
            level = Integer.valueOf(argument(call,"level"));
            Log.d("RFIDPlugin", "Power level received through method channel is "+String.valueOf(level));
          } catch(Exception e) {
            Log.d("RFIDPlugin", "level received with error in method channel and exception is "+e.toString());
          }
          device.setPowerLevel(level);
        }
        break;
