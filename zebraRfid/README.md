# ZebraRfid - Zebra API3 SDK + Datawedge Plugin

The ZebraRfid plugin integrates both the RFID API3 library and DataWedge into a single package. 
...
class _MyAppState extends State<MyApp> {
   ...
   Widget _listView() {
      ...
      Widget connectBtn;
      if (zebraRfid?.connectionStatus == Status.connected) {
         connectBtn = FloatingActionButton(
                 backgroundColor: Colors.lightGreenAccent,
                 onPressed: () => zebraRfid?.disconnect(),
                 child: const Text("Disconnect",
                         style: TextStyle(color: Colors.black, fontSize: 16)));
      } else {
         connectBtn = FloatingActionButton(
                 backgroundColor: Colors.redAccent.shade100,
                 onPressed: () => zebraRfid?.connect(),
   ...
   Widget build(BuildContext context) {
      ...
      return MaterialApp(
         home: Scaffold(
            appBar: AppBar(
               title: const Text('ZebraRfid Plugin Example'),
            ),
            body:
