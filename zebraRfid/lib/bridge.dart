...
class Bridge {
  ...
  final List<ZebraRfid> _listeners = [];
  ...
  factory Bridge({ZebraRfid? listener}) {
    _singleton ??= Bridge._();
    if (listener != null) {
  ...
  Bridge._() {
    ...
    _methodChannel = const MethodChannel("support.lib.rfid/method");

    // create event channel
    _eventChannel = const EventChannel("support.lib.rfid/event");
  ...
  bool contains(ZebraRfid listener) => _listeners.contains(listener);

  // listen for zebra events
  void addListener(ZebraRfid listener) {
  ...
  void removeListener(ZebraRfid listener) {
    if (contains(listener)) {
  ...
  void setPowerLevel(int level) {
    _methodChannel.invokeMethod("level", {"level": level});
  }
  ...
  void _eventListener(dynamic payload) {
    try {
      ...
      switch (event) {
        ...
        case Events.reading:
          print("RFID-READING event triggered");
          List<RfidTag> list = [];
          List<dynamic> tags = map["tags"];
          for (var i = 0; i < tags.length; i++) {
            var tag = Map<String, dynamic>.from(tags[i]);
